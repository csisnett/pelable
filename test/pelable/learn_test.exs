defmodule Pelable.LearnTest do
  ...
  describe "bookmarks" do
    ...
    test "list_bookmarks/0 returns all bookmarks" do
      ...
      assert Learn.list_bookmarks() == [bookmark]
    end

    test "get_bookmark!/1 returns the bookmark with given id" do
      bookmark = bookmark_fixture()
      assert Learn.get_bookmark!(bookmark.id) == bookmark
    end

    test "create_bookmark/1 with valid data creates a bookmark" do
      assert {:ok, %Bookmark{} = bookmark} = Learn.create_bookmark(@valid_attrs)
      assert bookmark.title == "some title"
    end

    test "create_bookmark/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Learn.create_bookmark(@invalid_attrs)
    end

    test "update_bookmark/2 with valid data updates the bookmark" do
      bookmark = bookmark_fixture()
      assert {:ok, %Bookmark{} = bookmark} = Learn.update_bookmark(bookmark, @update_attrs)
      assert bookmark.title == "some updated title"
    end

    test "update_bookmark/2 with invalid data returns error changeset" do
      bookmark = bookmark_fixture()
      assert {:error, %Ecto.Changeset{}} = Learn.update_bookmark(bookmark, @invalid_attrs)
      assert bookmark == Learn.get_bookmark!(bookmark.id)
    end

    test "delete_bookmark/1 deletes the bookmark" do
      bookmark = bookmark_fixture()
      assert {:ok, %Bookmark{}} = Learn.delete_bookmark(bookmark)
      assert_raise Ecto.NoResultsError, fn -> Learn.get_bookmark!(bookmark.id) end
    end

    test "change_bookmark/1 returns a bookmark changeset" do
      bookmark = bookmark_fixture()
      assert %Ecto.Changeset{} = Learn.change_bookmark(bookmark)
    end
  end
end
